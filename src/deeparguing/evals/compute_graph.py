"""Renders a PyTorch autograd backward graph via graphviz.

Vendored from torchviz (https://github.com/szagoruyko/pytorchviz), which is
unmaintained and fails to import on Python >= 3.12 due to its dependency on
the removed `distutils` module. Trimmed to drop the legacy `distutils`
