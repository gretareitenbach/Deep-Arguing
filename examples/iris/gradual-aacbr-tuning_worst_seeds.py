...
seeds = [4, 10, 15, 21, 30, 32, 38, 39, 52, 53, 59, 68, 75, 76, 91]
...
sweep_config = {
    ...
    "parameters": {
        "epochs": {"values": [1000, 3000]},
        "max_iters": {"values": [len(X_train)]},
        "use_symmetric_attacks": {"values": [False]},
        "lr": {"values": [1e-1, 5e-2]},
        # "temperature": {"values": [1.5, 1, 1e-1, 5e-1]},
        "temperature": {"values": [1e-1, 5e-2]},
...
sweep_id = wandb.sweep(sweep=sweep_config, project="gradual-aa-cbr-iris-worst-seeds")
