...
# pofe = fwe.FeatureWeightedExtractor(no_features, inital_weights=torch.tensor([1.0, 1, 1, 1], device=device))
# bsfe = fwe.FeatureWeightedExtractor(no_features, inital_weights=torch.tensor([1.0, 1, 1, 1], device=device))
pofe = fwe.FeatureWeightedExtractor(no_features)
bsfe = pofe
# bsfe = fwe.FeatureWeightedExtractor(no_features)
...
# base_score = lbs.LearnedBaseScore([bsfe], activation=torch.sigmoid)

# base_score = cbs.ConstantBaseScore(0.99)
...
print("TOTAL WITH F1 > 0.7:", totalf1, "/100")
