...
sweep_config = {
    "method": "grid",
    "name": f"seed-{torch_seed}",
    "metric": {"goal": "maximize", "name": "val_f1"},
    "parameters": {
        "epochs": {"values": [500, 3000]},
        "max_iters": {"values": [len(X_train)]},
        "use_symmetric_attacks": {"values": [False]},
        "lr": {"values": [3e-3, 8e-3, 3e-2, 8e-2]},
        "temperature": {"values": [1e-2, 5e-2, 1e-1, 5e-1]},
        "use_blockers": {"values": [True]},
        "initialisation_method": {"values": ["Xavier_uniform"]},
        "alpha": {"values": [0]},
        "beta": {"values": [0]},
        "gamma": {"values": [1e-3, 5e-3, 1e-2, 5e-2]},
    }
}

sweep_id = wandb.sweep(sweep=sweep_config, project="gradual-aa-cbr-iris-worst-seed")
...
def main():
    ...
    wandb_log = lambda loss: wandb.log({"train_loss": loss})
    accuracy, precision, recall, f1 = evaluate_model(model, X_train, y_train, X_DEFAULTS, Y_DEFAULTS, 
                                                    X_train, y_train, show_confusion=False, use_blockers=USE_BLOCKERS, use_symmetric_attacks=USE_SYMMETRIC_ATTACKS,  
    ...
    losses = static_train_model(model, X_train, y_train, 
                    ...
                    regularise_graph=regulariser, logger=wandb_log)
